...
**Action space (81):** 0–39 play card c; 40–79 *communicate* card c; 80 = pass.
Communication happens only in a one-round setup phase at the start of each game
(before any trick): each player, in turn order from the leader, either reveals
one eligible card (its only/highest/lowest of a color, type derived from the
hand) or passes. No mid-trick or per-trick communication. The policy learns what
to signal up front; a signal is cleared from the observation once its card is
played.
